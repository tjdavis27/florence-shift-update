...
const DRAFT_KEY = 'florenceShiftDraft.v210';
const LEGACY_DRAFT_KEYS = ['florenceShiftDraft.v200','florenceShiftDraft.v133'];
const UNDO_KEY = 'florenceShiftUndo.v210';
const LEGACY_UNDO_KEY = 'florenceShiftUndo.v200';
...
function collectCurrentShift(){
  return {
    ...
    giveawayItems: entries('giveawayRows'),
...
function applyDraft(draft){
  ...
  (draft.giveawayItems || []).forEach(x => addFromTemplate('giveawayTemplate','giveawayRows',x));
...
function buildWarnings(){
  ...
  entries('trainingRows').forEach((x,index) => {
    ...
    if(!x.accomplishment) warnings.push(`${label} is missing what they accomplished.`);
  });
  ...
  entries('giveawayRows').forEach((x,index) => {
    if(!(x.promotion || x.count || x.won || x.notes)) return;
    const label = `Giveaway ${index+1}`;
    if(!x.promotion) warnings.push(`${label} is missing the promotion.`);
    if(!x.count) warnings.push(`${label} is missing how many were given.`);
    if(!x.won) warnings.push(`${label} is missing what was won.`);
  });
...
function generate(options = {}){
  ...
  if(trainingItems.length){
    ...
    trainingItems.forEach(x => {
      lines.push(`Training: ${x.employee || 'Employee not listed'} - ${x.role || 'Role not listed'} - ${x.accomplishment || 'Accomplishment not listed'}`);
    });
  }
  ...
  const giveawayItems = entries('giveawayRows').filter(x => x.promotion || x.count || x.won || x.notes);
  if(giveawayItems.length){
    lines.push('');
    giveawayItems.forEach(x => {
      let line = `Giveaway: ${x.promotion || 'Promotion not listed'} - ${x.count || '?'} given - Won: ${x.won || 'Prize not listed'}`;
      if(x.notes) line += ` - ${x.notes}`;
      lines.push(line);
    });
  }
...
$('addGiveaway').addEventListener('click', () => addFromTemplate('giveawayTemplate','giveawayRows'));
